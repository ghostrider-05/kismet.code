...
export const writeNode = async (path: string, node: UnrealJsonReadFile, options: { 
    json?: boolean, 
    Package: string 
}): Promise<{ jsonNode?: Record<string, unknown>, Class?: JsonFile } | undefined> => {
...
export function writePackages (exportPath: string, files: { 
    ...
    Object.keys(files.classes).forEach(key => {
        const content = getExportFile(files.classes[key], <boolean>files.groupItems)
    ...
    if (files.json.length > 0) writeFile(resolve('.', './' + exportPath.concat(`/nodes.json`)), JSON.stringify(files.json))
}
