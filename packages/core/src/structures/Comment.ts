import { convertVariablesRecordToArray } from '@kismet.ts/shared'
...
export class Comment extends BaseSequenceItem {
    ...
    public override toJSON (): Record<string, KismetVariableValue> {
        const variables: [string, KismetVariableValue][] = [
            ...
            ['bDrawBox', KismetBoolean.toKismet(this.drawBox)],
            ['bFilled', KismetBoolean.toKismet(this.filled)],
            ['bTileFill', KismetBoolean.toKismet(this.tileFill)],
    ...
    public override toString (): string {
        ...
        const properties = convertVariablesRecordToArray(json)
            .map(([name, value]) => ({ name, value }))

        return this.setProperty(...properties).toString()
    }
}
